...
void Renderer::EndFrame() { m_Framebuffer.UnBind(); }
void Renderer::Render(project &Proj, int selectedSceneID) {
  ...
  glUniformMatrix4fv(glGetUniformLocation(DefaultShader, "u_VP"), 1, GL_FALSE,
                     glm::value_ptr(view));
  ...
  for (uint32_t entityID : activeScene->EntityIDs) {
    ...
    for (uint32_t compID : e->ComponentIDs) {
      ...
      if (c->Getname() == "Sprite") {
        ...
        glUniformMatrix4fv(glGetUniformLocation(DefaultShader, "u_Model"), 1,
...
void Renderer::RenderColliders(project &Proj) {
  ...
  glUniformMatrix4fv(glGetUniformLocation(GridShader, "u_VP"), 1, GL_FALSE,
                     glm::value_ptr(vp));
  ...
  for (uint32_t entityID : activeScene->EntityIDs) {
    ...
    for (uint32_t compID : e->ComponentIDs) {
      ...
      float vertices[] = {topLeft.x,     topLeft.y,     bottomLeft.x,
                          bottomLeft.y,  bottomRight.x, bottomRight.y,

                          bottomRight.x, bottomRight.y, topRight.x,
                          topRight.y,    topLeft.x,     topLeft.y};
      ...
      // Blue with 0.5 alpha
      glUniform4f(glGetUniformLocation(GridShader, "u_Color"), 0.0f, 0.0f, 1.0f,
                  0.5f);
      ...
      glBufferData(GL_ARRAY_BUFFER, sizeof(outlineVertices), outlineVertices,
                   GL_DYNAMIC_DRAW);
      glUniform4f(glGetUniformLocation(GridShader, "u_Color"), 0.0f, 0.0f, 1.0f,
                  1.0f);
      glLineWidth(2.0f);
...
void Renderer::RenderCameraGizmos(project &Proj) {
  ...
  float aspect =
      (float)Proj.Param.Resolution.width / Proj.Param.Resolution.height;
  ...
  for (uint32_t entityID : activeScene->EntityIDs) {
    ...
    glm::vec4 corners[4] = {model * glm::vec4(-halfWidth, -halfHeight, 0, 1),
    ...
    glUniform4f(glGetUniformLocation(GridShader, "u_Color"), 0.5f, 0.5f, 1.0f,
                1.0f);
...
void Renderer::RenderAxis() {
  ...
  glLineWidth(2.0f);
  glUniformMatrix4fv(glGetUniformLocation(GridShader, "u_VP"), 1, GL_FALSE,
                     glm::value_ptr(m_Camera.GetViewProjection()));

  // X axis
  glUniform4f(glGetUniformLocation(GridShader, "u_Color"), 1.0f, 0.0f, 0.0f,
              1.0f);
  glDrawArrays(GL_LINES, 0, 2);

  // Y axis
  glUniform4f(glGetUniformLocation(GridShader, "u_Color"), 0.0f, 1.0f, 0.0f,
              1.0f);
...
void Renderer::RenderGrid() {
  ...
  glUniform4f(glGetUniformLocation(GridShader, "u_Color"), 0.3f, 0.3f, 0.3f,
              1.0f);
  ...
  float left =
      m_Camera.Position.x - (m_Framebuffer.m_Width / 2.0f) / m_Camera.Zoom;
  float right =
      m_Camera.Position.x + (m_Framebuffer.m_Width / 2.0f) / m_Camera.Zoom;
  float bottom =
      m_Camera.Position.y - (m_Framebuffer.m_Height / 2.0f) / m_Camera.Zoom;
  float top =
      m_Camera.Position.y + (m_Framebuffer.m_Height / 2.0f) / m_Camera.Zoom;
  ...
  for (float x = floor(left / worldSpacing) * worldSpacing; x <= right;
       x += worldSpacing) {
  ...
  for (float y = floor(bottom / worldSpacing) * worldSpacing; y <= top;
       y += worldSpacing) {
