...
EditorUI::EditorUI(project &proj, bool &running)
    : Project(proj), renderer(nullptr), Run(running) {}

void EditorUI::Init(SDL_Window *window, SDL_GLContext glContext, project &Proj,
                    Renderer *renderer, bool &running, CoreRuntime *runtime,
                    Logger *Log, CommandSystem *cmdSystem) {
  ...
  io.Fonts->AddFontFromFileTTF(FilePaths::Resolve("assets/fonts/fontawesome-free-7.2.0-desktop/"
                               "otfs/Font Awesome 7 Free-Solid-900.otf").c_str(),
  ...
  panels.push_back(
      std::make_unique<Terminal>("Terminal", Proj, Log, CmdSystem));
  panels.push_back(
      std::make_unique<SceneManagerPanel>("SceneManager", Proj, SelectedScene));
  panels.push_back(
      std::make_unique<SceneParamPanel>("SceneParam", Proj, SelectedScene));
  panels.push_back(
      std::make_unique<ProjectSettingsPanel>(Proj.Param.name.c_str(), Proj));
  panels.push_back(std::make_unique<EntityList>("Entity List", Proj, Selection,
                                                SelectedScene));
  panels.push_back(
      std::make_unique<Inspector>("Inspector", Proj, Selection, renderer));
  panels.push_back(std::make_unique<Viewport>(
      "Viewport", Proj, Selection, renderer, SelectedScene, *runtime));
};
...
void EditorUI::RenderMenuBar(SDL_Window *window) {
  ...
  cfg.path = FilePaths::Assets();
  if (ImGui::BeginMainMenuBar()) {

    if (ImGui::BeginMenu("File")) {

      if (ImGui::MenuItem("Save Project")) {
        ImGuiFileDialog::Instance()->OpenDialog("SaveProjectDlg",
                                                "Save Project", ".json", cfg);
      }

      if (ImGui::MenuItem("Load Project")) {
        ImGuiFileDialog::Instance()->OpenDialog("LoadProjectDlg",
                                                "Load Project", ".json", cfg);
      }
    ...
    if (ImGui::BeginMenu("Edit")) {
      ...
      if (ImGui::BeginMenu(
              "Themes")) { // <-- this is now a submenu, not MenuItem
        struct ThemeOption {
          const char *name;
          void (*apply)();
        };
